...
const removeProjects = document.querySelectorAll(".removeProject");

removeProjects.forEach((removeProject) => {
  removeProject.addEventListener("click", ProjectRemove);
});
...
function projectAdj() {
  ...
  projects.forEach((project) => {
    project.getElementsByClassName("projectNum")[0].innerHTML = "Project " + i;
    i++;
  });
  var userProjects = document.querySelectorAll(".userProject");
  var i = 1;
  userProjects.forEach((userProject) => {
    userProject.getElementsByClassName("projectNum")[0].innerHTML =
      "Project " + i;
    i++;
  });
}
...
allprojects.forEach((project) => {
  ...
  project.addEventListener("click", ActiveElement);
});

const projectSec = document.querySelector(".projects");

projectSec.addEventListener("dragover", dragOver);
projectSec.addEventListener("drop", dragDrop);

const projectPas = document.getElementsByClassName("projectPa");

var x;
function ActiveElement() {
  x = document.activeElement.className;
  if (x == "projectPa") {
    allprojects.forEach((project) => {
